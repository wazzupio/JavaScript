...
function showCategory(category) {
    prodEl.insertAdjacentHTML('afterbegin', getProductMarkup(products[category]));
}
...
function getProductMarkup(product) {
    ...
    product.forEach(element => {
        markup += `<div>${element.name}</div><img src="${element.imageUrl}" alt=""><div>${element.price}</div><a href="https://example.com/producs/${element.id}">Подробнее</a>`;
    });
